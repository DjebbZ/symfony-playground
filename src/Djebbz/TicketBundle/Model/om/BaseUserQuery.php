...
abstract class BaseUserQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `name`, `created_at` FROM `user` WHERE `id` = :p0';
        try {
    ...
    public function filterByTicket($ticket, $comparison = null)
    {
        if ($ticket instanceof Ticket) {
            return $this
                ->addUsingAlias(UserPeer::ID, $ticket->getUserId(), $comparison);
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Djebbz\TicketBundle\Model\TicketQuery A secondary query class using the current class as primary query
     */
    public function useTicketQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinTicket($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Ticket', '\Djebbz\TicketBundle\Model\TicketQuery');
    }
